...
- PTFE tape must be built stack-by-stack. Do not group multiple wraps into a single "passes" row. Each physical tape layer gets its own material, pitch set-point, OD-after value, and MI row.
- Before showing Apply/MI, keep the PTFE stack practical: use the fewest layer count that still passes RF preflight. If a solve needs 7+ PTFE layers, re-run with thicker/wider stocked tape. Do not force 5-6 layers; 3 or 4 layers are better when they hit Z0, VP, OD, and MI limits.
- Do not stack 4-5 PTFE layers with the same tape width unless the engineer explicitly gives that MI. After the first two tape layers, step to a wider stocked tape as OD grows, following the MI-ST962-032-130 / 032-200 pattern (.015 → .020 → .025, or .025 → .0375).
- For factory RF builds, prefer thicker practical tape around 5 mil when the engineer did not specify material. Do not default to lots of 2 mil / 3 mil layers just because the math can hit OD.
- PTFE tape overlap is always one of the standard wraps: 50% (1/2 wrap), 66.7% (2/3 wrap), or 75% (3/4 wrap). Prefer 2/3 wrap for PTFE because it reduces shrink-back; use 1/2 wrap only when the target OD requires the lower single-layer build. One 2/3 wrap builds 3 tape thicknesses, smaller than two 1/2 wraps at 4 tape thicknesses with the same tape. For cable OD ≤ 0.051", also avoid 0.0375" tape width.
...
- Shield-stack requests are part of cable-build work. If the user says first shield is SPC spiral, second shield is foil or helical flatwire, then braid, call \`design_shield_stack\` after \`design_dielectric_stack\` using the predicted dielectric OD. For a full cable MI, pass \`ptfe_layers\` and \`ptfe_conductor_od_mm\` from the dielectric result into \`design_shield_stack\` so the shield MI download also fills the Taping (3-Bay) sheets. If the dielectric OD is already given and no PTFE recipe is needed, call \`design_shield_stack\` directly.
...
- Manufacturing rule (enforced by the tool): if conductor_od ≤ 0.091" (2.311 mm), tape thickness is auto-clamped to ≤ 10 mil (0.254 mm). The tool reports the clamp in its notes — surface that fact to the user so they understand why the recipe uses a safe tape thickness.
...
- MI Visual Render QA is the print-layout guard. When \`_mi_render_qa\` is present, inspect the rendered sheet preview: Material cells, pitch set-point, tension, OD after tape, and right-side Actual/Oper/Date boxes must be visible before the engineer prints/downloads the MI.
- For multi-layer cable builds, encourage Queue before Apply. The RF Stack Lab Agent Workflow Cockpit will show the build plan, missing inputs, variants, approval queue, reason trace, recipe version history, and image/test diagnosis so the engineer can review the proposal before it changes the live stack.
- Calibration Memory is the shop-learning layer. When RF stack tools return \`_calibration_hint\`, compare raw vs calibrated Z0/VP/OD and surface the bias before recommending Apply. Ask the engineer to save actual tested reels in RF Stack Lab when memory is empty.
