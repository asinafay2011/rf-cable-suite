...
/* ============================================================
   ...
   ─────────────────────────────────────────────────────────────
   Periodic tape wraps (PTFE dielectric tape, foil shield tape) on coax
   produce Bragg-style reflection notches. The fundamental notch lands at:

      f_n = n · c · VF / (2 · P)        (n = 1, 2, 3, …)

   where P is the axial period of the seam — controlled by tape WIDTH and
   OVERLAP. This is the "suckout" engineers see in S21 traces of taped
   semi-rigid and helically-wrapped low-loss cables. The trick on the
   factory floor: choose tape width so the lowest notch sits OUTSIDE the
   operating band.

   - Wider tape  →  bigger axial pitch  →  notch slides LEFT (lower freq)
   - Narrower tape → smaller axial pitch → notch slides RIGHT (higher freq)
   ============================================================ */
function SuckoutSim() {
  const [layer, setLayer]       = useState('ptfe');     // ptfe | foil
  const [width, setWidth]       = useState(12);          // tape width mm
  const [overlap, setOverlap]   = useState(25);          // overlap %
  const [layAngle, setLayAngle] = useState(20);          // helix angle from axis (°) – axial pitch shrinks for steeper wraps
  const [vf, setVf]             = useState(0.70);
  const [bandLo, setBandLo]     = useState(1000);        // MHz
  const [bandHi, setBandHi]     = useState(18000);       // MHz
  const [insertionLossBase, setInsertionLossBase] = useState(0.5); // dB/m baseline
  const [notchDepth, setNotchDepth] = useState(8);       // dB (depth at fundamental)

  // Axial pitch: tape advances W·(1−overlap)/cos(α_axial) per turn along axis
  // We assume a ribbon-like helical wrap where overlap is measured along the
  // tape's own axis (industry-typical), so axial advance ≈ W·(1−O/100)·cos(α).
  const pitch_mm = useMemo(() => {
    const w = Math.max(0.5, width);
    const o = Math.max(0, Math.min(50, overlap)) / 100;
    const cosA = Math.max(0.2, Math.cos((layAngle * Math.PI) / 180));
    return w * (1 - o) * cosA;
  }, [width, overlap, layAngle]);

  // f_n = n · 150_000 · VF / pitch_mm   [MHz]   (since c = 3e8 m/s, P in mm)
  const notches = useMemo(() => {
    const list = [];
    for (let n = 1; n <= 5; n++) {
      const f = (150000 * vf * n) / pitch_mm;
      if (f > 50_000) break;
  ...
  const verdict =
    inBand.length === 0
      ? { state: 'CLEAR', color: '#5eead4', detail: 'No notch falls inside the operating band — safe.' }
      : inBand.length === 1
      ? { state: 'WARNING', color: '#fbbf24', detail: `Order-${inBand[0].order} notch at ${inBand[0].f_mhz.toFixed(0)} MHz hits the band — engineering decision needed.` }
      : { state: 'FAIL', color: '#f87171', detail: `${inBand.length} notches in band — this tape geometry will create visible suckouts in S21.` };

  // Generate a synthetic |S21| sweep so the engineer can see the notch placement
  // 200 log-spaced points 100 MHz → 50 GHz
  const sweep = useMemo(() => {
    const arr = [];
    const f0 = 100, f1 = 50000;
    const N = 220;
    for (let i = 0; i < N; i++) {
      const t = i / (N - 1);
      ...
      for (const nt of notches) {
        const sigma = nt.f_mhz * 0.025;       // narrow notch ≈ 2.5% Q
        const dx = (f - nt.f_mhz) / sigma;
        notchSum += notchDepth * Math.exp(-dx * dx) / nt.order;  // higher-order notches shallower
      }
  ...
  // Suggest a tape width that pushes the FIRST notch above bandHi
  // bandHi = 150_000·VF / (W_safe·(1−O)·cosα) → W_safe = 150_000·VF / (bandHi·(1−O)·cosA)
  const suggestion = useMemo(() => {
    const o = Math.max(0, Math.min(50, overlap)) / 100;
    const cosA = Math.max(0.2, Math.cos((layAngle * Math.PI) / 180));
    const wMin = (150000 * vf) / (bandHi * (1 - o) * cosA) * 0.95; // 5% margin below first notch
    return wMin;
  }, [bandHi, vf, overlap, layAngle]);

  const layerLabel = layer === 'ptfe' ? 'PTFE dielectric tape' : 'Foil shield tape';
  const layerHint  = layer === 'ptfe'
    ? 'Helical PTFE tape forms the dielectric on semi-rigid / phase-stable cables (UT-141, SF-141, etc.). Period = tape seam repeat → Bragg notch in S21.'
    : 'Helical Al/PET foil shield wrap creates a periodic admittance discontinuity on the OUTSIDE of the cable — same Bragg physics, often a slightly shallower notch but visible in S21 above ~5 GHz.';

  return (
    <section className="space-y-5">
      <header className="space-y-1">
        <div className="font-mono text-[11px] tracking-[0.2em] uppercase text-[#c97b3f]">
        ...
        <h1 className="text-2xl md:text-3xl font-light tracking-tight" style={{ fontFamily: 'Bricolage Grotesque' }}>
          Where does my tape pitch put the suckout?
        </h1>
        <p className="text-[12px] md:text-[13px] text-[#a7b0b6] leading-relaxed max-w-3xl">
          On RF coax with helical tape (PTFE dielectric or foil shield), the tape's seam repeats every period <span className="font-mono text-[#fbbf24]">P</span>. That periodic perturbation Bragg-reflects at <span className="font-mono text-[#5eead4]">f<sub>n</sub> = n·c·VF / (2·P)</span> — the "suckout". Wider tape pushes the notch <span className="text-[#f87171]">left</span> (lower freq), narrower tape pushes it <span className="text-[#5eead4]">right</span>. Pick a width that lands the first notch <em>outside</em> your band.
        </p>
      </header>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="bg-[#12171a] border border-[#252e33] rounded p-4 space-y-3">
          <div className="font-mono text-[10px] uppercase tracking-wider text-[#fbbf24]">Tape geometry</div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="font-mono text-[10px] text-[#6b7479] uppercase tracking-wider mb-1 block">Layer</label>
              <select value={layer} onChange={(e) => setLayer(e.target.value)} className="w-full bg-[#0a0d0f] border border-[#252e33] rounded px-2 py-1.5 text-[12px] font-mono text-[#fbbf24]">
                <option value="ptfe">PTFE dielectric tape</option>
                <option value="foil">Foil shield tape</option>
              </select>
            </div>
            <SliderInput label="Width" value={width} onChange={setWidth} min={0.5} max={30} step={0.5} unit="mm" inchHint />
            <SliderInput label="Overlap" value={overlap} onChange={setOverlap} min={0} max={50} step={5} unit="%" />
            <SliderInput label="Lay angle" value={layAngle} onChange={setLayAngle} min={5} max={45} step={1} unit="° from axis" />
          </div>
          <div className="text-[11px] text-[#6b7479] italic leading-relaxed">{layerHint}</div>
        </div>

        <div className="bg-[#12171a] border border-[#252e33] rounded p-4 space-y-3">
          <div className="font-mono text-[10px] uppercase tracking-wider text-[#5eead4]">Cable + band</div>
          <div className="grid grid-cols-2 gap-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-[11px]">
            <div className="bg-[#0a0d0f] border border-[#252e33] rounded p-2">
              <div className="font-mono text-[9px] text-[#6b7479] uppercase">Axial pitch P</div>
              <div className="font-mono text-[14px] text-[#fbbf24] mt-0.5">{pitch_mm.toFixed(2)} mm</div>
              <div className="font-mono text-[10px] text-[#5eead4]">{(pitch_mm / 25.4).toFixed(4)}″</div>
            </div>
            <div className="bg-[#0a0d0f] border border-[#252e33] rounded p-2">
              <div className="font-mono text-[9px] text-[#6b7479] uppercase">1st notch</div>
              <div className="font-mono text-[14px] text-[#c97b3f] mt-0.5">
      ...
      {/* Verdict bar */}
      <div className="border rounded p-3 flex items-center gap-3 flex-wrap" style={{ borderColor: verdict.color + '60', background: verdict.color + '14' }}>
        <div className="font-mono text-[14px] font-medium tracking-wider" style={{ color: verdict.color }}>
          {verdict.state}
        </div>
        <div className="text-[12px] text-[#a7b0b6] flex-1 min-w-[200px]">{verdict.detail}</div>
        {verdict.state !== 'CLEAR' && suggestion < width && suggestion > 0.3 && (
          <button
            onClick={() => setWidth(parseFloat(suggestion.toFixed(1)))}
            className="font-mono text-[10px] uppercase tracking-wider px-3 py-1.5 rounded border bg-transparent hover:bg-[#1f1610]"
            style={{ color: '#5eead4', borderColor: '#5eead460' }}
          >
            → Try W = {suggestion.toFixed(1)} mm
          </button>
        )}
      </div>

      {/* Plot |S21| with notches and operating band */}
      <div className="bg-[#12171a] border border-[#252e33] rounded p-3">
        <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
          <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#5eead4]">|S21| vs frequency</div>
          <div className="font-mono text-[10px] text-[#6b7479]">band {(bandLo / 1000).toFixed(1)}–{(bandHi / 1000).toFixed(1)} GHz</div>
        </div>
        <ResponsiveContainer width="100%" height={260}>
          <LineChart data={sweep} margin={{ top: 8, right: 12, bottom: 4, left: -8 }}>
            ...
            <YAxis
              ...
              tick={{ fontSize: 10 }}
              domain={['auto', 0]}
              tickFormatter={(v) => `${v.toFixed(0)} dB`}
            />
            <Tooltip
            ...
            <ReferenceLine x={bandLo} stroke="#5eead4" strokeDasharray="3 3" label={{ value: 'band lo', fill: '#5eead4', fontSize: 9, position: 'top' }} />
            <ReferenceLine x={bandHi} stroke="#5eead4" strokeDasharray="3 3" label={{ value: 'band hi', fill: '#5eead4', fontSize: 9, position: 'top' }} />
            {/* Notch markers */}
            {notches.map((n) => (
              <ReferenceLine
                key={n.order}
                x={n.f_mhz}
                stroke={n.f_mhz >= bandLo && n.f_mhz <= bandHi ? '#f87171' : '#fbbf24'}
                strokeDasharray="4 2"
                label={{ value: `f${n.order}`, fill: n.f_mhz >= bandLo && n.f_mhz <= bandHi ? '#f87171' : '#fbbf24', fontSize: 9, position: 'top' }}
              />
            ))}
            <Line type="monotone" dataKey="s21" stroke="#c97b3f" dot={false} strokeWidth={1.6} />
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      {/* Notch table */}
      <div className="bg-[#12171a] border border-[#252e33] rounded p-3">
        <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#fbbf24] mb-2">Bragg notch frequencies</div>
        <table className="w-full text-[12px]">
          <thead>
            <tr className="font-mono text-[10px] text-[#6b7479] uppercase">
          ...
          <tbody>
            {notches.length === 0 && (
              <tr><td colSpan={5} className="text-[#6b7479] italic text-center py-3">Pitch too small — first notch &gt; 50 GHz, off chart.</td></tr>
            )}
            {notches.map((n) => {
              const inB = n.f_mhz >= bandLo && n.f_mhz <= bandHi;
              return (
                <tr key={n.order} className="border-t border-[#252e33] font-mono">
                  <td className="py-1.5 text-[#a7b0b6]">n = {n.order}</td>
                  <td className="py-1.5 text-right text-[#fbbf24]">{(n.f_mhz / 1000).toFixed(3)}</td>
                  <td className="py-1.5 text-right text-[#a7b0b6]">{n.f_mhz.toFixed(0)}</td>
                  <td className="py-1.5 text-right" style={{ color: inB ? '#f87171' : '#5eead4' }}>{inB ? 'YES — hits' : 'no'}</td>
                  <td className="py-1.5 text-right text-[#a7b0b6]">{(notchDepth / n.order).toFixed(1)} dB</td>
                </tr>
              );
            })}
          </tbody>
        </table>
        <div className="mt-2 text-[11px] text-[#6b7479] italic">
          Higher-order notches (n=2,3,…) are shallower (~1/n in amplitude) but can still bite if the band is wide. Real cables get partial spread from tape thickness variation, so notches are 50-200 MHz wide rather than infinitely sharp.
        </div>
      </div>

      {/* Width-vs-notch curve — visual intuition */}
      <WidthSweepPanel
        baseOverlap={overlap}
        baseLayAngle={layAngle}
        vf={vf}
        bandLo={bandLo}
        bandHi={bandHi}
        currentWidth={width}
      ...
      {/* Formula reference */}
      <div className="bg-[#12171a] border border-[#252e33] rounded p-4 space-y-2 text-[12px] text-[#a7b0b6] leading-relaxed">
        <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#5eead4] mb-1">Reference</div>
        <div className="font-mono text-[#fbbf24]">P = W · (1 − overlap) · cos(α)</div>
        <div className="font-mono text-[#fbbf24]">f<sub>n</sub> = n · c · VF / (2 · P) ≈ n · 150 000 · VF / P<sub>mm</sub>  [MHz]</div>
        <div>
          • <span className="text-[#f0ebe2]">{layerLabel}</span> — picking the right tape width is one of the cheapest ways to dodge a suckout. <em>You can&apos;t move the band, but you can move the notch.</em>
          <br />• On semi-rigid PTFE, factories run multiple tape heads with staggered widths to spread / smear the residual notch.
          <br />• Foil shield notches typically present at +5-15 dB above the dielectric notch (foil discontinuity is shallower).
        </div>
      </div>
    </section>
  );
...
function SliderInput({ label, value, onChange, min, max, step, unit, inchHint = false }) {
  return (
    <div>
      <label className="font-mono text-[10px] text-[#6b7479] uppercase tracking-wider mb-1 block">{label}</label>
      <div className="flex items-center gap-2">
        <input
          type="range"
          min={min}
          ...
          className="flex-1 accent-[#c97b3f]"
        />
        <input
          type="number"
          min={min}
          ...
          value={value}
          onChange={(e) => onChange(parseFloat(e.target.value))}
          className="w-16 bg-[#0a0d0f] border border-[#252e33] rounded px-1.5 py-0.5 text-[11px] font-mono text-[#fbbf24] text-right"
        />
        <span className="font-mono text-[10px] text-[#6b7479] w-12">{unit}</span>
      </div>
      {inchHint && <div className="font-mono text-[9px] text-[#5eead4] text-right mt-0.5">{(value / 25.4).toFixed(4)}″</div>}
    </div>
  );
}

function WidthSweepPanel({ baseOverlap, baseLayAngle, vf, bandLo, bandHi, currentWidth }) {
  const data = useMemo(() => {
    const arr = [];
    for (let w = 1; w <= 30; w += 0.5) {
      const o = Math.max(0, Math.min(50, baseOverlap)) / 100;
      const cosA = Math.max(0.2, Math.cos((baseLayAngle * Math.PI) / 180));
      const P = w * (1 - o) * cosA;
    ...
    return arr;
  }, [baseOverlap, baseLayAngle, vf]);

  return (
    <div className="bg-[#12171a] border border-[#252e33] rounded p-3">
      <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
        <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#fbbf24]">
          Notch frequency vs tape width
        </div>
        <div className="font-mono text-[10px] text-[#6b7479]">
          green = band, red lines = notch orders
        </div>
      </div>
      <ResponsiveContainer width="100%" height={220}>
        <LineChart data={data} margin={{ top: 8, right: 12, bottom: 4, left: 0 }}>
          <CartesianGrid stroke="#252e33" strokeDasharray="2 4" />
          <XAxis dataKey="w" stroke="#6b7479" tick={{ fontSize: 10 }} unit=" mm" />
          <YAxis
            stroke="#6b7479"
            tick={{ fontSize: 10 }}
            scale="log"
          ...
          <Tooltip
            ...
            labelFormatter={(w) => `W = ${w} mm`}
          />
          <ReferenceLine y={bandLo} stroke="#5eead4" strokeDasharray="3 3" label={{ value: 'band lo', fill: '#5eead4', fontSize: 9, position: 'left' }} />
          <ReferenceLine y={bandHi} stroke="#5eead4" strokeDasharray="3 3" label={{ value: 'band hi', fill: '#5eead4', fontSize: 9, position: 'left' }} />
          <ReferenceLine x={currentWidth} stroke="#fbbf24" strokeDasharray="2 2" label={{ value: `W=${currentWidth}`, fill: '#fbbf24', fontSize: 9, position: 'top' }} />
          <Line type="monotone" dataKey="f1" stroke="#c97b3f" strokeWidth={2} dot={false} name="n=1" />
          <Line type="monotone" dataKey="f2" stroke="#fbbf24" strokeWidth={1.2} dot={false} name="n=2" strokeDasharray="4 2" />
          <Line type="monotone" dataKey="f3" stroke="#a78bfa" strokeWidth={1.2} dot={false} name="n=3" strokeDasharray="2 2" />
        </LineChart>
      </ResponsiveContainer>
      <div className="mt-1 text-[11px] text-[#6b7479]">
        Read horizontally: pick a tape width such that the orange (n=1) line is <em>above</em> the band-high cyan dashed line for your application.
      </div>
    </div>
  );
}
