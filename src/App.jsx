...
const STEP_PATTERNS = [
  ...
  [/\b\d+\s*awg\b|\bsolid\s+(bare\s+)?(cu|copper)\b|fine\s+(cu|copper)|\b(cu|copper)\s+wire|precision.*(cu|copper|wire)|cu\s+\d+\.\d+\s*mm|\bcu\s+draw\b/i, { title: "Solid / precision Cu conductor", body: "Solid copper inner conductor. AWG (American Wire Gauge): lower number = thicker wire (18 AWG ≈ 1.02 mm, 14 AWG ≈ 1.63 mm). 'Fine Cu wire ±0.01 mm' means precision-drawn to very tight diameter tolerance — critical because even ±0.05 mm variation causes impedance ripple (VSWR bumps). Solid wire gives slightly lower RF loss than stranded (no skin-effect between strands). Drawback: breaks at repeated bending." }],
